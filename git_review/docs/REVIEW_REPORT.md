...
| **C-2** 판정 정확도 48.4% (E2E에서 발견) | ✅ 해결 | 접미사 유일 매칭 + 거부 규칙 #12~#14 추가. **48.4% → 98.7%** |
| 인증 방식 GitHub App → **OAuth App**(공개 저장소 한정) | ✅ 전환 | 사용자 결정. H-2 일부·M-6 동반 해소 |
| H-2 나머지 / M-1~M-5, M-7 / L-1~L-8 / D-2~D-12 | ⬜ 미착수 | 아래 "권장 처리 순서" 참조 |

### C-2. 판정 정확도 붕괴 (E2E에서 발견, 최초 3단계 검증에서는 미검출)

실계정 E2E로 이 프로젝트 자신을 검증하니 **준수율 48.4%**. 원인을 규칙별로 분해한 결과:

| 추출 규칙 | 존재 | 없음 | 정확도 |
|---|---|---|---|
| `tree-block` (R1) | 77 | 0 | 100% |
| `inline-code` (R3) | 17 | 82 | **17.2%** |

**실제 미구현은 0건**이었다. 82건의 정체는 ① 저장소에 실재하지만 문서가 앞을 생략해 적어 못 맞춘 것 67건(`AppHeader.tsx`, `components/EmptyState.tsx`), ② 애초에 파일이 아닌 것 14건(패키지명 `@octokit/rest`, MIME `application/x-ndjson`, 의사코드 예시 `a/b/c.ts`), ③ 접미사 중복 1건.

거부 규칙 #10·#11이 세그먼트 2개 이상을 면제하고 확장자 있는 단일 파일명을 통과시키므로, 규칙 설계상 R3를 걸러낼 수단이 없었다. C-1 수정 전에는 R1의 전량 오판정에 이 문제가 묻혀 있었다.

**조치**: 매칭에 3단계 "접미사 유일 일치"를 추가(후보 2개 이상이면 미매칭 유지)하고, 거부 규칙 #12(패키지명)·#13(MIME)·#14(예시 경로)를 신설. TECH_SPEC §3.2·§3.3·§4·§9 반영. 화면에는 "부분 경로 매칭" 배지와 저장소의 실제 경로를 함께 표시한다.

**남은 2건**: `route.ts`(저장소에 6개 존재 → 특정 불가, 의도된 미매칭) / `user/repos`(GitHub API 엔드포인트 표기).

> **교훈**: 최초 3단계 검증은 코드를 실행하지 않아 이 결함을 잡지 못했다. 리포트가 한계로 명시했던 "성공 경로 미검증"이 실제로 치명 결함을 숨기고 있었다.
