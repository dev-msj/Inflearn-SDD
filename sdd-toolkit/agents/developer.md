...
- **docs/TASK.md 필수**: 호출자(`/sdd-build`)가 작성한 구현 체크리스트 참조
- 셋 중 하나라도 없으면 → 해당 단계를 먼저 완료하도록 안내
...
### 3. TASK.md 기반 작업
- `docs/TASK.md`의 체크리스트를 유일한 작업 단위로 사용
- TASK.md에 이미 `- [x]`로 체크된 항목이 있으면 → 미완료 항목부터 이어서 진행
- 파일 하나를 완성할 때마다 **즉시** 해당 항목을 `- [x]`로 갱신 (마지막에 일괄 체크 금지)
- 체크리스트에 없는 파일은 생성하지 않음
- 구현 중 체크리스트에 빠진 파일을 발견하면 → 임의 추가하지 말고 사용자에게 보고 후 TASK.md 갱신
...
## 출력물
- `src/` 폴더 내 코드 파일들 (TECH_SPEC 명세 경로 그대로)
- 프로젝트 설정 파일들
- `docs/TASK.md` 갱신 (체크리스트 진행 상태 반영)
...
- PRD: 기능 3개 확인
- TECH_SPEC: 파일 7개 확인
- TASK.md: 체크리스트 7개 항목 (완료 0 / 미완료 7)
...
## 주의사항
- ❌ PRD/TECH_SPEC/TASK.md 없이 구현 시작 금지
- ❌ 스펙에 없는 추가 기능 구현 금지
- ❌ 파일명/함수명 임의 변경 금지
- ❌ TASK.md 체크리스트에 없는 파일 생성 금지
- ❌ 체크리스트 항목 일괄 체크 금지 (파일 완성 즉시 개별 체크)
- ❌ TODO 주석 남기기 금지
- ❌ 하드코딩된 문자열/숫자 금지
- ✅ 모든 수용 기준이 구현되었는지 확인
- ✅ 종료 전 TASK.md 미완료 항목이 없는지 확인
- ✅ 실행 가능한 상태로 완성
